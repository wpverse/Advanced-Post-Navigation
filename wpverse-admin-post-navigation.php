 ...
 */
/*
Plugin Name: 101Plugins Remixed Admin Post Navigation w/ Category Clouds
Version: 1.1
Plugin URI: http://101plugins.com/
Author: 101 Plugins
Author URI: http://101plugins.com/
Text Domain: wpverse-admin-post-navigation
Domain Path: /lang/
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Description: Adds links to navigate to the next and previous posts when editing a post in the WordPress admin, as well as easily navigate through post categories.
...
/*
	Copyright (c) 2012 101Plugins / WPVerse. Based on Admin Post Navigation plugin by Scott Reilly.
...
class c2c_AdminPostNavigation {
	...
	public static function register_post_page_hooks() {
		load_plugin_textdomain( 'wpverse-admin-post-navigation', false, basename( dirname( __FILE__ ) ) . DIRECTORY_SEPARATOR . 'lang' );

		self::$prev_text = __( '&larr; Previous', 'wpverse-admin-post-navigation' );
		self::$next_text = __( 'Next &rarr;', 'wpverse-admin-post-navigation' );
	...
	public static function do_meta_box( $post_type, $type, $post ) {
		...
		if ( in_array( $post->post_status, $post_statuses ) )
			add_meta_box( 'adminpostnav', sprintf( __( '%s Navigation', 'wpverse-admin-post-navigation' ), ucfirst( $post_type ) ), array( __CLASS__, 'add_meta_box' ), $post_type, 'side', 'core' );
	}
	...
	public static function add_meta_box( $object, $box ) {
		...
		if ( $prev ) {
			...
			$display .= '<a href="' . get_edit_post_link( $prev->ID ) . '" id="admin-post-nav-prev" title="' .
				esc_attr( sprintf( __( 'Previous %1$s: %2$s', 'wpverse-admin-post-navigation' ), $context, $post_title ) ) .
		...
		if ( $next ) {
			...
			$display .= '<a href="' . get_edit_post_link( $next->ID ) .
				...
				esc_attr( sprintf( __( 'Next %1$s: %2$s', 'wpverse-admin-post-navigation' ), $context, $post_title ) ).
		...
		$categories = get_the_category( $post_ID );
		 foreach($categories as $category) {
 		 $url = admin_url( 'edit.php?post_type=post&category_name=', 'http' );
  		 $display .= '<a href="' . $url . $category->slug . '" title="' . sprintf( __( "View all posts in %s" ), $category->name ) . '" class="admin-post-nav-next add-new-h2" ' . '>' . $category->name.'</a> &nbsp;';
  		 } 
